...
/*
 * $Log: AbstractXMLResourceType.java,v $
 ...
 */
public class AbstractXMLResourceType extends AbstractResourceType {

    /**
     * Constructor.
     * 
     * @param typeIn The java type that is mapped.
     * @param soapTypeIn The soap type that is mapped.
     */
    public AbstractXMLResourceType(final Class typeIn, 
                                   final String soapTypeIn) {
        super();
        
        this.set(typeIn, soapTypeIn);
    }
    ...
    public Entry getEntry(final SoapConfig configIn, 
        ...
        if (value != null && value instanceof XMLResource) {
            
            try {
                
                XMLResource sres = (XMLResource) value;
                
                Entry edid = new Entry(entry, "documentId");
                edid.setValue(sres.getDocumentId());
                
                if (sres instanceof SixdmlResource) {
                    Entry ename = new Entry(entry, "name");
                    ename.setValue(((SixdmlResource) sres).getName());
                }
                
                // encode the xml document
                Document doc = (Document) sres.getContentAsDOM();
                configIn.getEncEntry(
                        entry, "content", doc, Document.class, 
                        objMap, xsdNS, xsiNS);
