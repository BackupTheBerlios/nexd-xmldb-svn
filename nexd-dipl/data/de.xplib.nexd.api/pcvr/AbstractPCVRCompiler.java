...
import _de.xplib.nexd.api.VirtualResource;
import de.xplib.nexd.engine.xml.jaxp.DocumentBuilderFactoryImpl;

/**
 * <p>
 * This class creates a <code>{@link _de.xplib.nexd.api.VirtualResource}</code>
 * from a given <code>{@link _de.xplib.nexd.api.pcvr.PCVResource}</code> instance
 * .</p>
 * <p>It uses the DOM <code>{@link org.w3c.dom.Document}</code> that is returned
 * by the <code>{@link _de.xplib.nexd.api.pcvr.PCVResource#getContentAsDOM()}
...
public abstract class AbstractPCVRCompiler implements PCVRCompiler {
    ...
    public AbstractPCVRCompiler() throws XMLDBException {
        ...
        try {
            this.builder = new DocumentBuilderFactoryImpl()
                    .newDocumentBuilder();
    ...
    private String getBase64Value(final String encValueIn) {
        return new String(Base64.decodeBase64(encValueIn.getBytes()));
    }
    ...
    protected void parseValue(final String dataIn, final Node refIn)
        ...
        String data = new String(Base64.decodeBase64(dataIn.getBytes()));
